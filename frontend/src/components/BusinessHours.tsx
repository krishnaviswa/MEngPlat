...
export function BusinessHours({ hours }: BusinessHoursProps) {
  ...
  if (entries.length === 0) {
    return <p className="text-sm text-muted">Hours not listed</p>;
  }
    ...
    <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm">
      {entries.map(([day, value]) => (
        <div key={day} className="contents">
          <dt className="font-medium capitalize text-muted">{day}</dt>
          <dd className="text-muted">{typeof value === "string" ? value : JSON.stringify(value)}</dd>
        </div>
      ))}
    </dl>
  );
