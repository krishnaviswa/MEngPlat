...
| M-71 | Merchant/Customer | Review collection flow (public QR/link wizard, no gating)          | `/collect/[businessId]` (shop by slug or UUID); `/businesses/{slug}/review` thanks screen | `/collect/:slug` (UUID or slug) + merchant QR/share sheet + Android App Links on `/collect` | `partial` | S-040 (web UI unchanged), S-059 / S-106 / **S-118** (mobile). App installed + verified App Links → Flutter collect; otherwise web `/collect`. Mobile resolves UUID via `GET /businesses/id/{id}` or slug via `GET /businesses/{slug}` (404 slug then id). WhatsApp QR stays `wa.me`. Camera→app needs SHA-256 in `assetlinks.json` matching the installed APK; `localhost` scans never open the app. |
