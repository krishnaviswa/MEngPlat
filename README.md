...
| M-71 | Merchant/Customer | Review collection flow (public QR/link wizard, no gating)          | `/collect/[businessId]` (shop by slug or UUID); `/businesses/{slug}/review` thanks screen | `/collect/:slug` (UUID or slug) + merchant QR (`merchanthub://app/collect/...`) + website share URL + Android App Links | `partial` | S-040 (web UI unchanged), S-059 / S-106 / **S-118**. Mobile **QR** opens the app via custom scheme (no fingerprint). **Share** still sends web `/collect`. HTTPS camera scan of the **web** dashboard QR still needs `assetlinks.json` SHA-256. WhatsApp QR stays `wa.me`. |
