...
# Optional — same Web OAuth client ID as backend GOOGLE_CLIENT_ID (shows Continue with Google):
...
changes. `GOOGLE_CLIENT_ID` is the Web OAuth client (empty hides Gmail on Login/Register); Android
SHA-1 setup is under [Google sign-in](#google-sign-in).
...
| M-04 | Auth              | Google / Gmail sign-in                                             | `/login`, `/register` (`GoogleSignInButton`)    | `/login`, `/register` (`GoogleSignInButton`)                           | `implemented`   | S-029; skips MFA; hidden if `GOOGLE_CLIENT_ID` unset. APK/AAB CI: `vars.MOBILE_GOOGLE_CLIENT_ID` |
...
| Local dev       | `docker compose up --build`. Flutter Merchant Home tiles use owned-business counts and do not wait on `GET /dashboard/merchant/{id}` or topic clustering. That dashboard route now uses two aggregate SQL round trips (month volume + one `COUNT FILTER` KPI query). Sideload APKs need GitHub variable `MOBILE_GOOGLE_CLIENT_ID` or Gmail stays hidden. |
