...
| Role              | Email                                                       | Password       | Mobile OTP phone |
| ----------------- | ------------------------------------------------------------ | -------------- | ---------------- |
| Admin             | `admin@merchanthub.ai`                                       | `admin12345ok` | `+919000000000` |
| Merchant          | `merchant@example.com`                                       | `merchant1234` | `+919000000001` |
| Merchant (pending) | `merchant2@example.com`                                     | `merchant1234` | `+919000000002` |
| Customer          | `customer@example.com`                                       | `customer1234` | `+919000000003` |
| Chennai/US demo (×2) | `demo.customer1@example.com` … `demo.customer2@example.com` | `demo12345abc` | — |


**Password login requires an authenticator app.** Seeded demo accounts share TOTP secret `JBSWY3DPEHPK3PXP` (add to Google Authenticator / Authy as a time-based account). Gmail/Google sign-in does not require TOTP. **Mobile OTP:** choose that method, use the demo phone for the role (merchant is `9000000001`, admin is `9000000000`). **Temporary Mobile OTP shortcut:** after **Send SMS code**, enter `123456` (`DEMO_PHONE_OTP`, default in backend). Remove via [docs/agents/REMOVE-ME-demo-fixed-otp.md](docs/agents/REMOVE-ME-demo-fixed-otp.md). Hosted `localreview.co.in` needs a **backend** deploy with this code — a new login UI alone is not enough. A new number cannot self-register as admin.
...
| Actor        | Wants                              | Can do                                                                                                                                                                                                                                                                                          |
| ------------ | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Customer** | find businesses worth trusting     | register/login (Authenticator or Mobile OTP, Google), search & browse, view profiles, rate 1–5, write reviews with photos, like/report, favorite, password reset. **Not built:** edit/delete own reviews (parity M-40/M-41).                                                                                                                                          |
...
Architecture files for this layer: [ADR-007](docs/agents/adrs/ADR-007-transactional-email-port.md), [ADR-008](docs/agents/adrs/ADR-008-razorpay-featured-fee.md), [ADR-010](docs/agents/adrs/ADR-010-featured-sku-admin-approve.md), [ADR-011](docs/agents/adrs/ADR-011-phone-otp-sms.md), [ADR-012](docs/agents/adrs/ADR-012-whatsapp-cloud-api-port.md), [ADR-019](docs/agents/adrs/ADR-019-partner-review-channel-mock.md) (partner review channel), [ADR-009](docs/agents/adrs/ADR-009-web-functional-e2e.md) (how we *prove* the loop), plus `.cursor/rules/ai-and-integrations.mdc` / `backend/app/services/CLAUDE.md`. Partner channel strategy: [`docs/PARTNER_REVIEW_CHANNEL_STRATEGY.md`](docs/PARTNER_REVIEW_CHANNEL_STRATEGY.md).
...
sequenceDiagram
    ...
    alt Email / password (Authenticator)
        ...
        User->>Frontend: Choose Mobile OTP + role (customer / merchant / admin)
        Frontend->>API: POST /auth/phone/request then /auth/phone/verify
        API->>DB: Match User.phone or create customer/merchant
        API-->>Frontend: JWT (skips TOTP). New admin self-register is 403
    else Google sign-in
...
Merchants create listings at `/merchant/businesses/new` (wrapped in `RequireAuth role="merchant"`). The shared `BusinessForm` posts to `POST /businesses`; new rows start with `status=pending`. Country defaults to `IN` in the form (backend model default is `US` if omitted) and is a bundled ISO-3166 `<select>`; State is a cascading `<select>` of that country's subdivisions (S-084). Street address and city are plain text. There is no address lookup, geocode button, or manual latitude/longitude entry (S-084 dropped the geocoder; the manual lat/lng inputs were removed outright since no merchant used them — see §14). `latitude`/`longitude` stay optional fields on the API/DB, just with no UI to set them today.
...
| POST   | `/auth/reauth`           | Bearer           | Step-up check before email change (any role) or merchant phone/national ID change (**S-114**). Body: exactly one of `password`, `totp_code`, `phone`+`otp_code`, or Google `credential`. Returns `{ "reauth_token": "..." }` (JWT `type=reauth`, ~5 minutes). 401 invalid credentials; 400 wrong method mix. |
...
| POST   | `/admin/users/{id}/suspend`    | Admin | Set `is_active=false` + `audit_logs` row. Idempotent. `400` if `id` is the caller or another admin, `404` unknown id. **Does not delete** the user or related reviews/businesses (**S-115**) |
...
Suspended/reactivated accounts are always `customer` or `merchant` — an admin can never suspend themselves or another admin. Existing login/refresh/`get_current_user` already reject `is_active=false` (S-034). There is **no** `DELETE /admin/users/{id}`: suspend keeps the user row, reviews, businesses, and audit logs (S-115). Account erasure / GDPR anonymize stays in the legal program (§14), not an admin button.
...
| ------------------- | -------------------------------------------------------- |
| `LoginForm`         | `email`, `password`, `authMethod`, `loginRole`, MFA step, `error`, `loading` |
...
The home page loads cities, categories, stats, and (for the voices band) reviews from the API on SSR. Bands in order: brand-first hero (search + CTAs over a storefront photo plane — no stats overlays; the photo plane stays dark for contrast), live trust metrics (`GET /businesses/stats/summary` including `total_cities`), neighborhood and category indexes with counts derived from the listed catalog (category index uses `surface-raised` / `ink` so it follows the theme toggle), featured listings with optional `ai_merchant_summary` suggestion blurbs, real review voices with AI summaries labeled as suggestions, how-it-works, and a merchant AI CTA (brand wash in light mode, deeper brand surface in dark — not a permanently navy band). Featured section title and city-scoped grid come from `GET /businesses/cities` — not hardcoded place names.
...
| `Footer.tsx`               | Multi-column site map: Discover, merchants, Account, Support (S-087). Semantic `surface-raised` / `ink` / `muted` / `border` so it follows the theme toggle; ≥44px link hit areas with overlapping vertical margin so lists stay compact (S-122 follow-up) |
...
| `PhoneOtpPanel.tsx`        | Phone OTP request/verify (S-044); shown only when **Mobile OTP** is selected (S-092) |
| `AuthMethodToggle.tsx`     | Authenticator vs Mobile OTP chooser on `/login` and `/register` (S-092)     |
| `RegisterForm.tsx`         | Registration; same method chooser as login; customer/merchant only (S-092) |
...
**Authority split** — Figma leads for visual decisions (colour, spacing, type, component structure); code leads for behaviour (state, data fetching, routing). When the two disagree on a visual value, Figma wins.

- **Web:** file **MerchantHub AI — Design System**, key `X0XXhJiwW8SxFdMf39n2t3`. Mirror hex into `tailwind.config.ts`.
- **Mobile:** separate file **MerchantHub AI — Mobile**, key `rk4RnruVFTpKdIsgGJIt9w` (390pt job screens). Mirror into Flutter `MhTheme` / `MhTokens` (`mobile/lib/ui/`). Do not clone 1440 web templates onto the phone.

Add the token in Figma first, then mirror. New mobile slices must name a Figma Mobile frame + states (default / empty / loading / error) and a hub slot or route — not “looks like `/admin` on web.”
...
**Convention going forward:** new components should reach for these 5 tokens instead of hardcoded `bg-white` / `text-gray-*` grey utilities, so they theme correctly for free. The global `Footer` and home category / merchant CTA bands follow that rule (they are not permanently dark). Tone-carrying colors (sentiment badges, star-rating fill, hover chrome) intentionally stay as explicit `dark:` pairs rather than folding into a generic token, since they carry meaning beyond surface/text role. `Charts.tsx` (Recharts) is the one place tokens can't reach — its SVG props read a `CHART_COLORS` light/dark map keyed off `useTheme().resolvedTheme` instead. The home hero photo plane stays a dark overlay for image contrast.
...
| Register / login / JWT / Google | `test_api.py`, `test_auth_hardening.py`, `test_google_auth.py`, `test_google_config.py`, `test_s018_s020_login_profile.py` | `LoginForm`, `RegisterForm`, `AlreadySignedIn`, `RequireAuth`, `AuthMethodToggle`, `lib/__tests__/api.test.ts` | `register_google_auth_test.dart`, `google_sign_in_button_test.dart`, `google_client_id_test.dart`, `post_login_path_test.dart`, `widget_test.dart` | `test_flow_anonymous.py`, `test_flow_customer.py`, `test_token_security.py`, `journeys/test_journey_anonymous.py` (S-124) | TR-S-018, 067, 092, 098, 102, **116**, **124** |
| Logout / session | `test_auth_logout.py`, `test_cache_blocklist.py`, `test_dependencies_blocklist.py` | `RequireAuth`, `Navbar` | `app_shell_test.dart`, `post_login_path_test.dart` | `test_token_security.py`, `journeys/test_journey_customer.py` (S-124 logout blocklist) | TR-S-018, 067, **124** |
...
| Step-up / reauth | `test_reauth.py` | `MerchantNationalIdCard` | `profile_screen_test.dart` | `journeys/test_journey_merchant.py` (S-124 KYC reauth) | TR-S-107, TR-S-114, **124** |
| Profile / national ID / avatar | `test_national_id.py`, `test_avatar.py`, `test_s018_s020_login_profile.py` | `ProfilePage`, `MerchantNationalIdCard`, `ui/Avatar` | `profile_screen_test.dart` | `journeys/test_journey_customer.py` (S-124) | TR-S-019, 043, 056, 070, 071, 072, 085, 095, **124** |
| List / edit business, address, hours, onboarding | `test_businesses_mine.py`, `test_business_address_reverify.py`, `test_business_service_summary.py`, `test_business_processing_status.py` | `BusinessForm`, `BusinessHours`, `GooglePlacePicker`, `OnboardingGuidancePanel`, `BusinessPhotoManager` | `business_editor_screen_test.dart`, `country_state_test.dart`, `business_hours_test.dart`, `my_business_ids_provider_test.dart` | `test_flow_merchant.py`, `journeys/test_journey_merchant.py` (S-124: create via Enter, address OTP, photos) | TR-S-069, 073, 074, 075, 079, 084, **124** |
| Business list/detail / slugs / cache | `test_businesses_slugs_filter.py`, `test_businesses_cache_invalidation.py`, `test_cache_lock.py` | `BusinessCard`, `search/page` | `business_list_screen_test.dart`, `business_detail_screen_test.dart`, `business_card_test.dart`, `search_controller_test.dart` | `test_flow_anonymous.py` | TR-S-028, 066, **117** |
| Photos / storage | `test_photos.py`, `test_storage.py` | `BusinessPhotoManager` | (detail/editor photo paths) | — | TR-S-051, 075 |
| Reviews write / moderate | `test_reviews.py`, `test_content_moderation.py` | `ReviewForm`, `ReviewCard`, `ui/Textarea` | `review_form_sheet_test.dart`, `review_card_test.dart`, `reviews_controller_test.dart`, `rating_stars_test.dart` | `test_flow_customer.py`, `journeys/test_journey_customer.py` (S-124: submit, Enter-from-title, rating gate), `journeys/test_journey_admin.py` (S-124 moderation cycle) | TR-S-023, 030, **124** |
...
| Collect review / merchant QR | — | `CollectQrCard`, `app/collect/[businessId]/page` (UI unchanged S-118; gamified flow **S-119**: `components/collect/gamified/*`, `page.gamified.test.tsx`), `mobile/features/reviews/gamified/*`; `public/.well-known/assetlinks.json`; inline auth-on-submit **S-121**: `components/collect/__tests__/InlineAuthStep.test.tsx`, `page.test.tsx`, `page.gamified.test.tsx` | `collect_review_screen_test.dart`, `collect_review_screen_gamified_test.dart` (**S-119**), `share_review_link_sheet_test.dart` (+ **S-120** print-share case), `app_shell_test.dart` (**S-120** QR shop-resolution cases), `business_collect_resolve_test.dart`, inline auth-on-submit **S-121**: `inline_auth_step_test.dart`, `collect_review_inline_auth_test.dart` | `journeys/test_journey_anonymous.py` (S-124 inline auth-on-submit) | TR-S-037–040, 059, 077, **118, 119, 120, 121, 124** |
...
| Search / categories / home marketing | `test_categories_search.py` | `search/page`, `FeaturedGrid`, `CategoryBadges`, `home/SocialProofRail`, `home/ProblemSection` | `home_screen_test.dart`, `search_controller_test.dart` | `test_flow_anonymous.py`, `test_smoke_compose.py` | TR-S-041, 047, 064, 081, 099, 114, **116**, **117** |
| Merchant dashboard / analytics / trends | `test_dashboard.py`, `test_dashboard_deltas.py`, `test_benchmark.py` | `MerchantDashboard`, `Dashboard`, `Charts`, `BenchmarkCard`, `ui/StatCard` | `merchant_dashboard_screen_test.dart`, `dashboard_hub_test.dart`, `platform_series_chart_test.dart`, `role_home_screen_test.dart` | `test_flow_merchant.py`, `journeys/test_journey_merchant.py` (S-124: date range, refresh, CSV, AI disclaimer, reply) | TR-S-022, 033, 060, 063, 100, **117**, **124** |
| Featured boost / payments | `test_payments.py` | `FeaturedBoostPanel`, `admin/AdminPaymentPanel` | `merchant_dashboard_screen_test.dart` (grow/boost) | `journeys/test_journey_merchant.py`, `journeys/test_journey_admin.py` (S-124 boost + admin approve/refund) | TR-S-036, 042, 062, **124** |
| Admin platform / queues / ops nav | `test_admin_platform.py`, `test_admin_platform_asgi.py`, `test_admin_browse.py`, `test_admin_browse_asgi.py`, `test_dashboard.py` (platform keys) | `app/admin/page`, `AdminOpsNav`, `PendingBusinessQueue`, `AllBusinessesQueue`, `AllReviewsQueue`, `drilldown-back-links`, `AdminBackLink` | `admin_home_screen_test.dart`, `admin_route_gating_test.dart`, `admin_copy_test.dart` | `test_flow_admin.py`, `test_rbac_matrix.py`, `journeys/test_journey_admin.py` (S-124) | TR-S-021, 034, 079, 086, 091, 093, 101, **115**, **117**, **124** |
| Admin users / categories | (browse + platform files) | `AdminUserPanel`, `AdminCategoryPanel`, `app/admin/page` (Users helper), `app/admin/businesses/page` | `admin_users_screen_test.dart`, `admin_categories_screen_test.dart` | `journeys/test_journey_admin.py` (S-124: category create + 409, user suspend/reactivate) | TR-S-041, 080, 082, 083, **115**, **124** |
...
| Support contact / tickets | `test_support_tickets_and_reports.py` | `SupportTicketForm`, `app/support/page`, `admin/AdminSupportQueue`, `Footer` | `support_screen_test.dart`, `admin_support_queue_screen_test.dart` | `journeys/test_journey_anonymous.py`, `journeys/test_journey_admin.py` (S-124) | TR-S-087, 088, 094, **124** |
| Shop / business reports | `test_support_tickets_and_reports.py` | `ReportShopButton`, `admin/AdminBusinessReportsQueue` | `admin_business_reports_screen_test.dart`, `business_detail_screen_test.dart` | `journeys/test_journey_admin.py` (S-124) | TR-S-089, 094, **124** |
...
| AI suggestions / topics / providers | `test_ai_contract.py`, `test_ai_gateway.py`, `test_ai_registry.py`, `test_ai_topics.py`, `test_ai_openai_family.py`, `test_ai_anthropic.py`, `test_ai_provider_config.py`, `test_ai_startup_validation.py` | `AIInsights` | (merchant insights panel via dashboard tests) | `journeys/test_journey_merchant.py` (S-124 disclaimer copy) | TR-S-037–040, 049, **124** |
...
| Full role journeys — predefined-value human-driven catalogue (S-124) | — | — | — | `journeys/test_journey_anonymous.py`, `journeys/test_journey_customer.py`, `journeys/test_journey_merchant.py`, `journeys/test_journey_admin.py`, `test_selectors_smoke.py` (opt-in `E2E=1`; `E2E_FULL=1` for the coverage guard) | TR-S-124 |
...
| Predefined-value human-driven catalogue (S-124: `form_data.py` + `human.py` + `journeys/`, Enter-to-submit, dual oracles, selector canary, coverage guard) | **In place** (opt-in `E2E=1`; `E2E_FULL=1` for the session coverage guard); still manual `web-e2e.yml` |
...
# Optional override — otherwise Flutter loads the Web client ID from GET /api/v1/auth/google-config:
...
changes. `GOOGLE_CLIENT_ID` dart-define is an optional Web OAuth override; if empty, Login/Register fetch `GET /api/v1/auth/google-config`. Sideload CI (`.github/workflows/mobile-build-apk.yml`) bakes `WEB_BASE_URL` from GitHub variable `MOBILE_WEB_BASE_URL` (or the `web_base_url` workflow input). That must be the **public website origin** you already use in the browser for `/collect` — not the Railway `frontend-production-…` hostname and not the API host. The Dart default remains `http://localhost:3000` for local `flutter run`. GitHub-hosted sideload APKs are signed with `mobile/android/sideload.keystore` (alias `sideload`); paste SHA-1 `C0:6D:89:4E:25:E3:92:DA:DD:DE:95:75:5E:8B:68:72:7F:B8:A4:D6` on the Android OAuth client for package `com.merchanthub.merchanthub_mobile`. Android SHA-1 setup is under [Google sign-in](#google-sign-in). **S-118 App Links:** the Android intent host is derived from `WEB_BASE_URL` at build time; paste the sideload/Play **SHA-256** into [`frontend/public/.well-known/assetlinks.json`](frontend/public/.well-known/assetlinks.json).
...
Primary chrome is `AppShell` (`StatefulShellRoute` + `NavigationBar`): Explore / Favorites (customer) /
...
Google sign-in (skips MFA; shown when dart-define or `GET /auth/google-config` provides a client ID) and editable `/account/profile`.
...
| M-04 | Auth              | Google / Gmail sign-in                                             | `/login`, `/register` (`GoogleSignInButton`)    | `/login`, `/register` (`GoogleSignInButton`)                           | `implemented`   | S-029/S-102; skips MFA; dart-define optional; else `GET /auth/google-config`. Android OAuth + SHA-1 still required. **S-116:** mobile button is filled (surface + strong border + G mark) so it does not blend into `MhCanvas`. |
...
| M-11 | Chrome            | Bottom nav / primary shell                                         | — (web uses top nav)                            | `AppShell` `StatefulShellRoute` + `NavigationBar`                      | `implemented`   | S-027 / ADR-005 (S-103 per-tab stacks). **S-116:** tab-root Android back goes `/home` instead of exiting; nested routes still pop first. |
| M-12 | Chrome            | Brand / home link                                                  | Navbar → `/`                                    | Account `MerchantHub AI` → Explore                                     | `implemented`   | S-027                                                        |
| M-13 | Discovery         | Home hero + marketing sections                                     | `/`                                             | `/home` compact hero + browse (S-114); web keeps long `/`              | `partial`       | S-014 (web); S-064 then **S-114** mobile density — problem/how-it-works/CTA stay web-only |
| M-14 | Discovery         | Trust metrics                                                      | `/` (`TrustMetrics`)                            | `/home` trust-metrics row                                              | `implemented`   | S-064; omitted when stats fail, same as web |
| M-15 | Discovery         | City index                                                         | `/` (`CityIndex`)                               | `/home` Neighborhood invite → expand → `/businesses?city=`             | `implemented`   | S-064; **S-116** invite cards (S-114 segmented toggle superseded) |
| M-16 | Discovery         | Category index                                                     | `/` (`CategoryIndex`)                           | `/home` Category invite → expand → `/businesses?category=`             | `implemented`   | S-064; **S-116** invite cards (S-114 segmented toggle superseded) |
| M-17 | Discovery         | Featured businesses + photos                                       | `/` (`FeaturedGrid`)                            | `/home` featured grid (`BusinessCard`)                                 | `implemented`   | S-064; S-114 truncates suggestion blurb |
| M-18 | Discovery         | Review voices / how-it-works / merchant CTA                        | `/`                                             | `/home` compact voices; how-it-works + CTA **web only**                | `partial`       | S-064; **S-114** dropped long mobile copy |
...
| M-49 | Account           | Settings (profile link + logout)                                   | `/settings`                                     | `/account` + profile; merchant: My shop / QR / Grow                    | `implemented`   | S-027; **S-114** merchant shortcuts |
| M-50 | Merchant          | Dashboard shell + multi-business                                   | `/merchant/dashboard`                           | `/merchant`                                                            | `implemented`   | S-031                                                        |
| M-51 | Merchant          | Stats tiles + sentiment chart                                      | Dashboard                                       | Shop hub tiles; Insights hero numbers + bars                           | `implemented`   | S-031                                                        |
...
| M-54 | Merchant          | Create / edit business                                             | `/merchant/businesses/…`                        | `/merchant/businesses/new` + `…/:id/edit`                              | `implemented`   | S-031; S-084 Country/State dropdowns on web and mobile; city picker + type-in; S-073 address OTP on mobile edit |
...
| M-57 | Admin             | Admin home + platform stats                                        | `/admin`                                        | `/admin` list metrics, sticky jump chips, queue rows                   | `implemented`   | S-031; Hub list+chart layout (not pastel tiles)              |
...
| M-62 | Admin             | Platform time-series charts                                        | `/admin`                                        | `/admin` one touchable `fl_chart` line (7/30/90d + series chooser)   | `implemented` | S-034 (web), S-061 (mobile) — both Accepted                            |
...
| M-64 | Admin             | User suspend / reactivate                                          | `/admin`                                        | `/admin/users`                                                         | `implemented` | S-034 (web), S-061 (mobile) — both Accepted. **S-115:** helper copy that suspend keeps records (no delete) on web Users and mobile `/admin/users`. |
...
| M-71 | Merchant/Customer | Review collection flow (public QR/link wizard, no gating)          | `/collect/[businessId]` (shop by slug or UUID); `/businesses/{slug}/review` thanks screen | `/collect/:slug` + one https QR/share (same web collect URL) + Open-in-app | `partial` | S-040 (web UI unchanged), S-059 / S-106 / **S-118**. Scan opens the **website** collect page (phone cameras are https-only). In-app form is **Open review form in this app**. WhatsApp QR stays `wa.me`. **S-119** (2026-08-22): optional tap-through, one-question-at-a-time "gamified" collect UI on both web and mobile, gated off by default behind `NEXT_PUBLIC_GAMIFIED_REVIEW` (Railway, web) / `MOBILE_GAMIFIED_REVIEW` (GitHub Actions repo variable, mobile) — the two flags are independent, flip both to change behavior everywhere; route/QR/App-Links contract unchanged either way. Mobile v1 has no chip step (parity gap, intentional). **S-120** (2026-08-22): mobile-only fix — Account screen's "Share review QR" no longer guesses `shops.first` for merchants with >1 shop, and the share sheet gained a "Share QR to print" PNG export via the native OS share sheet (web's `CollectQrCard` print button untouched). **S-121** (2026-08-29): inline auth-on-submit, both platforms simultaneously. Composing the review (stars/chips/text) stays fully open — zero login gate at any step; an unauthenticated customer is asked to sign in only at Submit, via an inline step rendered in place (all three existing methods: Google, phone OTP as the default, email+password) — no route push to `/login`, and the already-composed review is never lost. This retires an undocumented behavior where an unauthenticated submit silently sent the customer to `/login?next=...`, and `redirectAfterAuth` ignored the `next` param and landed them back on their role's home page with the review gone; that path is simply never reached from this flow now (`/login` itself, e.g. via the header "Log in" link, is unchanged). |
...
| M-76 | Home              | Social proof rail (businesses-using-MerchantHub strip)             | `/` (`SocialProofRail`)                         | `/home` social-proof rail                                              | `implemented`   | S-047 (web); S-064 (mobile). **S-116:** prev/next + peek + visible scrollbar (no hidden overflow). |
| M-77 | Home              | Problem section (three named product gaps, numbered layout)        | `/` (`ProblemSection`)                          | web `/` only (mobile omitted S-114)                                    | `partial`       | S-047 (web); S-064 then **S-114** |
...
| M-81 | Admin             | Processing business status (admin-triggered visibility state between Pending and Approve/Suspend) | `/admin` Pending queue "Start review"/"Return to pending" + status badges | Admin Home merged pending+processing queue (`Start review` / `Return to pending`) | `implemented`   | S-079 (web); **S-093** (mobile, 2026-08-19) |
| M-82 | Admin             | Admin Users panel search (by name/email)                            | `/admin` Users panel                            | `/admin/users` search                                                  | `implemented`   | S-080 (web); **S-093** (mobile) |
| M-83 | Admin             | Admin Categories panel search (by name)                              | `/admin` Categories panel                       | `/admin/categories` search                                             | `implemented`   | S-081 (web); **S-093** (mobile) |
| M-84 | Admin             | Categories panel repositioned to top of `/admin`; distinct "Add category" error messages (409/401-403/network) | `/admin` Categories panel                       | Manage categories above WhatsApp; `admin_copy.dart` 409/401/network    | `implemented`   | S-082 (web); **S-093** (mobile) |
...
Mobile traffic outweighs web traffic for this product. Original P0–P4 / Tiers 1–5 tracker rows are closed. **S-093–S-095 (2026-08-19)** closed the remaining web-only rows **M-81–M-91**. **S-096–S-101** restyle Flutter as job-sized mobile (Figma Mobile `rk4RnruVFTpKdIsgGJIt9w`, not 1:1 web layouts). Still `partial`: M-10 chrome, M-65 in-app reset, M-71 cold QR, M-74 OTP chooser. `future`: FCM. This section
...
| 4    | Admin ops                           | ~~M-62~~, ~~M-63~~, ~~M-64~~ — **Done** (S-061, 2026-08-18); ~~M-81–M-86, M-90~~ — **Done** (S-093, S-095, 2026-08-19) | Lower urgency — admins are more likely to be at a desk than on mobile. **Tier 4 fully closed including later web-only admin rows.** |
...
M-65 (in-app password reset) stays `partial`. M-71 (cold QR → native `/collect`) stays `partial` after **S-118**: Android `/collect` App Links + UUID lookup are in the app, but Digital Asset Links SHA-256 must be pasted into `frontend/public/.well-known/assetlinks.json` (sideload: `keytool -list -v -keystore mobile/android/sideload.keystore -alias sideload`; Play: App signing certificate) before Android 12+ will prefer the app over the browser. FCM (M-47) remains `future`.
...
| S-092 | Authenticator vs Mobile OTP chooser (all roles) | 1 Foundation | In Progress |
| S-093 | Mobile admin queue polish (M-81–M-86) | 4 Dashboards | **Accepted** |
| S-094 | Mobile support + shop reports (M-87–M-89) | 5 Polish | **Accepted** |
| S-095 | Mobile ops console + avatar upload (M-90, M-91) | 5 Polish | **Accepted** |
| S-096 | Mobile native theme + UI kit | 5 Polish | **Accepted** |
| S-097 | Shared empty / loading / error chrome | 5 Polish | **Accepted** |
| S-098 | Auth screens restyle | 5 Polish | **Accepted** |
| S-099 | Customer / guest surface restyle | 5 Polish | **Accepted** |
| S-100 | Merchant hub + Insights / Reviews / Grow | 4 Dashboards | **Accepted** |
| S-101 | Admin Home restyle | 4 Dashboards | **Accepted** |
| S-114 | Mobile Home density + merchant find-shop + step-up ID | 5 Polish | In Progress |
...
| Local dev       | `docker compose up --build`. Flutter Merchant Home tiles use owned-business counts and do not wait on `GET /dashboard/merchant/{id}` or topic clustering. That dashboard route now uses two aggregate SQL round trips (month volume + one `COUNT FILTER` KPI query). Sideload APKs show Gmail when the production API's `GET /auth/google-config` returns a client ID (optional bake-time `MOBILE_GOOGLE_CLIENT_ID`). Android OAuth client + debug SHA-1 still required for the account picker. |
...
| Navbar shell polish (S-122) | **Accepted.** Current-section cue (`aria-current` + underline + heavier weight) via `ui/NavLink`; ≥ 44 px touch targets on every navbar and footer link; sub-`md` secondary links collapse into a keyboard-accessible `NavbarMobileMenu` (toggle + `aria-expanded`, Escape / outside-click close). Follow-up: footer + home category/merchant bands use semantic / `dark:` pairs (no permanently navy chrome); footer lists drop extra `space-y-2` on top of 44px rows; navbar Sign Up fill is an inner label so the hit box is not a tall solid pill. Mobile: native `NavigationBar` already covers this job — §12 M-92 `future`. No Flutter file changes (M-10 / M-87: mobile is not a cloned web footer). |
...
| **Thin tests / incomplete browser e2e** | Layers 1–3 (pytest + RTL) are real. Playwright journeys exist (`E2E=1`); GitHub run is **manual** [`web-e2e.yml`](.github/workflows/web-e2e.yml) (artifact traces). pytest/Jest workflows are also dispatch-only. **S-124** adds an exhaustive predefined-value, Enter-to-submit Playwright catalogue across every customer / merchant / admin route and form (`backend/tests/e2e/journeys/`, `form_data.py`, `human.py`) — still opt-in, still manual `web-e2e.yml`, still **not** a merge gate. See §11. |
...
| **Listings often lack map coordinates** | **S-084** dropped Nominatim lookup from the merchant address form (product decision: typed address + Country/State dropdowns, no live geocode). The manual latitude/longitude inputs that replaced it were themselves removed (2026-08-22, product decision: no merchant was using them) — `latitude`/`longitude` stay optional columns/API fields, just with no way to set them from either form today. Most new/edited listings will have no coordinates and will not appear in `POST /maps/nearby` distance search or on the search map until a future geocode mechanism is built. |
| **Mobile Home first paint still waits on a bundled payload** | **S-117** isolates search rebuilds, chart/map `RepaintBoundary`, and display-sized image decode. `homePayloadProvider` still `Future.wait`s public list/cities/categories/stats (plus social-proof and featured fetches) before `/home` leaves the skeleton. Splitting/deferring that payload is a later slice. |
...
| Legal: ToS, Privacy, cookie CMP, GDPR program, lawyer-reviewed retention (incl. national ID) | Lawyers, not a PR. **S-115** product policy: admins suspend only (`is_active`); records stay. Full account erase/anonymize is this legal bucket, not an admin Delete button. |
...
4. Leftovers: structured logs, hermetic test DB, CSP, **S-026** cookies, Azure stub, Play Store phase 5, mobile parity (§12). Admin Hub and Merchant Insights now use list+chart (not pastel tiles); Shop / Home / Explore still use `MhStatTile` / job tiles.
...
| `DEMO_PHONE_OTP`              | `123456`                                                                 | Temporary fixed SMS code (any SMS provider). Empty disables. Remove: `docs/agents/REMOVE-ME-demo-fixed-otp.md` |
