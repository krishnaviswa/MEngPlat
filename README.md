...
**Password login requires an authenticator app.** Seeded demo accounts share TOTP secret `JBSWY3DPEHPK3PXP` (add to Google Authenticator / Authy as a time-based account). Gmail/Google sign-in does not require TOTP. **Mobile OTP** is an alternative on `/login` (choose **Mobile OTP**, then the demo phone). With `SMS_PROVIDER=mock` the 6-digit code is logged by the backend, not sent to a real handset. A new number cannot self-register as admin.
