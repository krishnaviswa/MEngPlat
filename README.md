...
| Local dev       | `docker compose up --build`. Flutter Merchant Home tiles use owned-business counts and do not wait on `GET /dashboard/merchant/{id}` or topic clustering. That dashboard route now uses two aggregate SQL round trips (month volume + one `COUNT FILTER` KPI query). |
