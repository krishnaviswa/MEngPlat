...
async def consume_otp(phone: str, code: str) -> bool:
    ...
    Temporary demo bypass: when `DEMO_PHONE_OTP` is non-empty, that exact code
    is accepted (any SMS provider). See docs/agents/REMOVE-ME-demo-fixed-otp.md.
    """
    from app.config import get_settings

    settings = get_settings()
    demo = (settings.demo_phone_otp or "").strip()
    if demo and code.strip() == demo:
        return True
