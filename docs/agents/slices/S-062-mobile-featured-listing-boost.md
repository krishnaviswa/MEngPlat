...
- [x] All AC verified in test report (`TR-S-062-mobile-featured-listing-boost.md`) — all 10 AC Pass
- [x] UX matches notes above, including the badge position on `BusinessCard` and the "complete
      purchase on web" affordance for AC 1
- [x] No §8 Frontend guide update needed — no new package, reuses S-059's `url_launcher`/
      `AppConfig.webBaseUrl` pattern exactly, not a new reusable pattern
- [x] `README.md` §12 Web ↔ mobile feature parity tracker — M-66 row updated to `partial`
- [x] `README.md` §12 Mobile parity roadmap Tier 3 annotated with M-66 partially closed
...
| 2026-08-18 | Builder | Implemented per spec. New `payments_repository.dart` (two read-only methods, matches `dashboard_repository.dart`'s `try/on DioException` pattern). New `featured_boost_panel.dart`: `Future.wait` loads both SKUs and placement, renders live SKU tiles, status line (`active` → "Active until {endsAt}"; `awaitingApproval` → "Payment received — awaiting admin approval"; else "Not currently featured"), static "Purchase is completed on the web merchant dashboard for now" copy, and an `openWebCheckoutButton` labeled "Buy a featured boost on the web dashboard" with a launch-failure snackbar. `merchant_dashboard_screen.dart`: panel inserted after the Edit/Share-link row, gated on `business.status == approved`. `merchant_providers.dart`: `paymentsRepositoryProvider` added. `business_card.dart`: `Featured` `Chip` opposite the category chip, shown only when `isFeatured == true`. `business_list_screen.dart`: always-shown disclaimer line above results. No `pubspec.yaml` change. `flutter analyze` 0 issues pre-Tester. |
| 2026-08-18 | Tester | All 10 AC verified Pass. `flutter analyze` 0 issues, `flutter test` 222/222 (210 pre-existing + 12 new, 0 regressions). Independently re-verified the AC 8 checkout boundary by grep: `featuredCheckoutApiV1PaymentsFeaturedCheckoutPost` and `razorpay` both zero hits in `mobile/lib`/`pubspec.yaml`. Dedicated tests confirm the "web dashboard" wording is present in both the button label and the static note (not relying on the button alone), and that a captured-but-unapproved payment never renders as "Active until...". **Test-infra fix, no production code change:** the new panel's real network call wasn't stubbed in `merchant_dashboard_screen_test.dart`'s shared pump helper, causing a pre-existing S-059 test to hang; fixed by adding a `_FakePaymentsRepository` fixture (defaults to "never featured") to the helper. Test plan/report: `TP-S-062-mobile-featured-listing-boost.md`, `TR-S-062-mobile-featured-listing-boost.md`. Recommendation: Ship, `partial` (checkout deferral is the slice's deliberate, un-closed half). |
| 2026-08-18 | PM | Reviewed the Tester's report — all 10 AC Pass, `partial` adopted for M-66 (checkout-initiation remains explicitly deferred). `README.md` §12 M-66 row set to `partial`, rollup corrected (`implemented` 54 · `partial` 4 · `unimplemented` 15 · `n/a` 6 · `future` 1 · total 80), Tier 3 roadmap row annotated with M-66 done (`partial`). §14/§16 updated. No §8 Frontend guide update needed — no new package, reuses S-059's `url_launcher`/`webBaseUrl` pattern exactly. DoD checklist complete. **Status: Specified → Accepted.** |
