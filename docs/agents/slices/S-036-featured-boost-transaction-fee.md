...
| 2026-08-15 | Tester | Stub TR (file mapping only; pytest not run). |
| 2026-08-15 | PM | Accepted against stub TR-S-036 (invalid Tester pass — Builder-era). |
| 2026-08-15 | Tester | Shot 1 rewrite of TR-S-036: 9/9 AC mapped; `test_payments.py` **30 passed** (venv, DB-free; Railway `DATABASE_URL` not used); S-036 Jest files pass. Alembic `d5e6f7a8b9c0` `create_type=False` + single `create(checkfirst=True)` confirmed. Recommendation **Ship**. Status left as-is (PM owns Accepted). |
| 2026-08-15 | PM | **Accepted** on Tester shot 1 (`TR-S-036-featured-boost.md`, Ship, 9/9). Replaces the earlier stub-TR accept. Spot-check: ₹499 / 7-day SKU, featured-first search + paid-placement copy (not AI), dashboard active-until, fail-does-not-feature, admin disable/refund + ledger, no customer charge, mock/no-PAN, no grants. Non-blocking: no live Postgres/ASGI/Docker/Razorpay capture; 401 and second-merchant 404 rely on existing Depends/`get_owned_business`; Alembic enum `create_type=False` is ops, not an AC fail; S-037 Jest chart failures stay Test shot 2. README §12 M-66, §14 Payments, §16 fee numbers already match shipped web. |
